...
static void     add_to_prop_buf(xmlBufferPtr buf_ptr, 
                                ...
                                xmlChar * propName);
static void     parse_into_words(swish_ParseData * parse_data, 
                                xmlChar * string, 
                                int len, 
                                xmlChar * metaname,
...
)
{
     ...
     * indiv characters yet */
        ...
        if (nheaders >= min_headers && strlen((char *) line) == 0)
        {        
    ...
    )
{
    ...
    if (metaname == NULL)
        metaname = parse_data->metastack->flat;
